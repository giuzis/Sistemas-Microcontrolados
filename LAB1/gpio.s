...
; enable clock to GPIOF at clock gating register
			...
			MOV		R1, #GPIO_PORTA                 ;Seta o bit da porta F
...
; 2. Limpar o AMSEL para desabilitar a analógica
            ...
            STR     R1, [R0]						;Guarda no registrador AMSEL da porta J da memória
            LDR     R0, =GPIO_PORTQ_AHB_AMSEL_R		;Carrega o R0 com o endereço do AMSEL para a porta F
            ...
            STR     R1, [R0]					    ;Guarda no registrador AMSEL da porta F da memória
			LDR     R0, =GPIO_PORTM_AHB_AMSEL_R		;Carrega o R0 com o endereço do AMSEL para a porta F
            STR     R1, [R0]					    ;Guarda no registrador AMSEL da porta F da memória
...
; 3. Limpar PCTL para selecionar o GPIO
            ...
            STR     R1, [R0]                        ;Guarda no registrador PCTL da porta J da memória
            LDR     R0, =GPIO_PORTQ_AHB_PCTL_R      ;Carrega o R0 com o endereço do PCTL para a porta F
            ...
            STR     R1, [R0]                        ;Guarda no registrador PCTL da porta F da memória
			LDR     R0, =GPIO_PORTM_AHB_PCTL_R      ;Carrega o R0 com o endereço do PCTL para a porta F
            STR     R1, [R0]                        ;Guarda no registrador PCTL da porta F da memória
			
; 4. DIR para 0 se for entrada, 1 se for saída
            LDR     R0, =GPIO_PORTA_AHB_DIR_R		;Carrega o R0 com o endereço do DIR para a porta F
			MOV     R1, #0XF0						;Colocar 0 no registrador DIR para funcionar com saída
            STR     R1, [R0]						;Guarda no registrador
			; O certo era verificar os outros bits da PF para não transformar entradas em saídas desnecessárias
            LDR     R0, =GPIO_PORTQ_AHB_DIR_R		;Carrega o R0 com o endereço do DIR para a porta J
            MOV     R1, #0x0F               		;Colocar 0 no registrador DIR para funcionar com saída
            STR     R1, [R0]						;Guarda no registrador PCTL da porta J da memória
            ...
            MOV     R1, #0x30               		;Colocar 0 no registrador DIR para funcionar com saída
            STR     R1, [R0]						;Guarda no registrador PCTL da porta J da memória
			LDR     R0, =GPIO_PORTM_AHB_DIR_R		;Carrega o R0 com o endereço do DIR para a porta J
            MOV     R1, #0x40               		;Colocar 0 no registrador DIR para funcionar com saída 01000000
            STR     R1, [R0]						;Guarda no registrador PCTL da porta J da memória
...
; 6. Setar os bits de DEN para habilitar I/O digital
			...
			MOV     R1, #2_01000000                     ;Ativa os pinos PQ0 a PQ3 como I/O Digital      
            STR     R1, [R0]                            ;Escreve no registrador da memória funcionalidade digital
...
; -------------------------------------------------------------------------------
; Função PortA_Output
; Parâmetro de entrada: R0 --> se os BIT0 estão ligado ou desligado
; Parâmetro de saída: Não tem
...
PortA_Output
			...
			LDR R2, [R1]
			BIC R2, #2_11110000                     ;Primeiro limpamos o bit do lido da porta R3 = R3 & 11111110
...
; -------------------------------------------------------------------------------
; Função PortM_Output
; Parâmetro de entrada: R0 --> se é para acender ou desligar os LEDs
; Parâmetro de saída: Não tem
PortM_Output
			LDR	R1, =GPIO_PORTM_AHB_DATA_R		    ;Carrega o valor do offset do data register
			;Read-Modify-Write para escrita
			LDR R2, [R1]
			BIC R2, #2_01000000                     ;Primeiro limpamos o bit do lido da porta R3 = R3 & 11111110
			ORR R0, R0, R2                          ;Fazer o OR do lido pela porta com o parâmetro de entrada
			STR R0, [R1]                            ;Escreve na porta F o barramento de dados dos pinos F0
			BX LR
